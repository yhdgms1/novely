...
type ValidAction =
  ...
  | [
      "input",
      [string, (meta: ActionInputOnInputMeta) => void, ActionInputSetup?],
    ]
...
type ActionProxyProvider<Characters extends Record<string, Character>> = {
  choice: {
    (
      ...choices: (
        | [Unwrappable, ValidAction[]]
        | [Unwrappable, ValidAction[], () => boolean]
      )[]
    ): ValidAction;
    (
  ...
  condition: <T extends string | true | false>(
    condition: () => T,
    variants: Record<
      T extends true ? "true" : T extends false ? "false" : T,
      ValidAction[]
    >,
  ) => ValidAction;
  exit: () => ValidAction;
  ...
  function: (
    fn: (restoring: boolean, goingBack: boolean) => Thenable<void>,
  ) => ValidAction;
...
type DefaultActionProxyProvider = ActionProxyProvider<
  Record<string, Character>
>;
type GetActionParameters<
  T extends Capitalize<keyof DefaultActionProxyProvider>,
> = Parameters<DefaultActionProxyProvider[Uncapitalize<T>]>;
