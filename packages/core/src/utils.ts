...
const isEmpty = (val: unknown): val is {} => {
  return (
    typeof val === "object" && !isNull(val) && Object.keys(val).length === 0
  );
};
...
const isUserRequiredAction = (
  ...
  return (
    action === "custom" &&
    meta[0] &&
    (meta[0] as unknown as CustomHandler).requireUserAction
  );
};
...
const getLanguage = (languages: string[], language = navigator.language) => {
  if (languages.includes(language)) {
    ...
    (language = languages.find((value) => navigator.languages.includes(value))!)
  ) {
    return language;
  }
