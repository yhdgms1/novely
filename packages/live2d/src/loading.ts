...
const { promise: library, resolve: libResolve, reject: libReject } = Promise.withResolvers<typeof import('./pixi-live2d')>()
...
const log = (msg: string) => {
  ...
  console.log(
    `%c[${time}s] ${msg}`,
    'background: #f34361; color: #fff; padding: 4px 8px;'
  );
};
...
const fetchLibrary = once(async () => {
  ...
  try {
    libResolve(await import('./pixi-live2d'));
    log('Library Load');
  } catch {
    libReject();
  }
